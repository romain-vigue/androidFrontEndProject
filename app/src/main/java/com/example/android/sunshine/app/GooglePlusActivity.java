/**
 * Role of this code: do the sign up/sign in of patients and doctors
 *
 * This code is derived from: gplus-quickstart-android
 * available here under open source license:
...
import com.example.android.sunshine.app.data.GET_isAdoctor_task;
...
public class GooglePlusActivity extends FragmentActivity implements
        ...
        CheckBox.OnCheckedChangeListener, GoogleApiClient.ServerAuthCodeCallbacks, AsyncResponseIsAdoctor, AsyncResponseSignUp {


    private final String LOG_TAG = GooglePlusActivity.class.getSimpleName();
    private String isAdoctor="patients";
    private String SignIn = "false";
    private String[] params;
    private Context mySignInContext;
    POST_SignUp_task contextSignUp = new POST_SignUp_task();
    GET_isAdoctor_task contextIsAdoctor = new GET_isAdoctor_task();
    ...
    private SignInButton mSignUpAsPatientButton;
    private SignInButton mSignUpAsDoctorButton;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //Get context
        contextSignUp.delegate = this;
        contextIsAdoctor.delegate = this;
        mySignInContext =this;

        mSignInButton = (SignInButton) findViewById(R.id.sign_in_button);
        mSignUpAsDoctorButton = (SignInButton) findViewById(R.id.sign_up_as_doctor_button);
        mSignUpAsPatientButton = (SignInButton) findViewById(R.id.sign_up_as_patient_button);

        //Set Button color
        mSignUpAsDoctorButton.setColorScheme(SignInButton.COLOR_LIGHT);
        mSignUpAsPatientButton.setColorScheme(SignInButton.COLOR_LIGHT);

        //Set button text
        setGooglePlusButtonText(mSignInButton,getString(R.string.sign_in_label));
        setGooglePlusButtonText(mSignUpAsDoctorButton,getString(R.string.sign_up_doctor_label));
        setGooglePlusButtonText(mSignUpAsPatientButton,getString(R.string.sign_up_patient_label));
...
//        mCirclesListView = (ListView) findViewById(R.id.circles_list);
        ...
        mSignUpAsPatientButton.setOnClickListener(this);
        mSignUpAsDoctorButton.setOnClickListener(this);
        ...
        if (!mGoogleApiClient.isConnecting()) {
            ...
            switch (v.getId()) {
                case R.id.sign_in_button:
                    SignIn = "true";
                    Log.d(LOG_TAG, "sign in "+"isAdoctor = "+isAdoctor);
                    //mStatus.setText(R.string.status_signing_in);
                    mSignInProgress = STATE_SIGN_IN;
                    mGoogleApiClient.connect();
                    break;

                case R.id.sign_up_as_patient_button:
                    SignIn = "false";
                    isAdoctor ="patients";
                    Log.d(LOG_TAG, "sign in "+"isAdoctor = "+isAdoctor);
                    mSignInProgress = STATE_SIGN_IN;
                ...
                case R.id.sign_up_as_doctor_button:
                    SignIn = "false";
                    isAdoctor ="doctors";
                    Log.d(LOG_TAG, "sign in "+"isAdoctor = "+isAdoctor);
                    mSignInProgress = STATE_SIGN_IN;
...
//        }
    ...
    public void onConnected(Bundle connectionHint) {
        ...
        //delete the database
        mySignInContext.getContentResolver().delete(DbContract.DataEntry.CONTENT_URI,null,null);
...
//        findViewById(R.id.layout_server_auth).setVisibility(View.GONE);
        ...
        String first_name = currentUser.getName().getGivenName();
        String last_name = currentUser.getName().getFamilyName();
        String user_id = currentUser.getId();

        String user_name = currentUser.getDisplayName();
        Log.e(LOG_TAG, "onConnected _____________________________________ = "+ first_name);
        params = new String[] {isAdoctor,user_id,user_email, SignIn,first_name,last_name};
        Log.d(LOG_TAG, "onConnected = "+ SignIn);

        if (SignIn.equals("true")){
            GET_isAdoctor_task myNewIsAdoctorTask = new GET_isAdoctor_task();
            myNewIsAdoctorTask.delegate = contextIsAdoctor.delegate;
            myNewIsAdoctorTask.execute(params);

        } else {
            POST_SignUp_task myNewPostSignUpTask = new POST_SignUp_task();
            myNewPostSignUpTask.delegate = contextSignUp.delegate;
            myNewPostSignUpTask.execute(params);
...
//        mCirclesAdapter.notifyDataSetChanged();
    ...
    @Override
    public void processFinishIsAdoctor(String isAdoctorResult) {

        if (isAdoctorResult.equals("true")){
            params[0] = "doctors";
            Intent intent = new Intent(mySignInContext, MainActivity.class);
            ...
            editor.putString("params",sb.toString());
            editor.apply();

            //intent.putExtra("params", params);
            startActivity(intent);
        }
        if (isAdoctorResult.equals("false")){
            params[0] = "patients";
        ...
        if (isAdoctorResult.equals("Not existing")){
            AlertDialog.Builder alert = new AlertDialog.Builder(mySignInContext);

            alert.setTitle(R.string.SignInFailure_title);
            alert.setMessage(R.string.SignInFailure_message);
    ...
    @Override
    public void processFinishSignUp(String signUpResult) {
        //Show Sign up failure:
        if ("500".equals(signUpResult)) {
            AlertDialog.Builder alert = new AlertDialog.Builder(mySignInContext);

            alert.setTitle(R.string.SignUpFailure_title);
            alert.setMessage(R.string.SignUpFailure_message);
            ...
            alert.setTitle(R.string.SignUpSuccess_title);
            alert.setMessage(R.string.SignUpSuccess_message);
